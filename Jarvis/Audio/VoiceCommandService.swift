...
/// to the matching action when the user says something like "Jarvis info" or
/// "Jarvis update".
///
...
final class VoiceCommandService {
    ...
    private func handle(partial: String) {
        ...
        // Slice after the last occurrence of "jarvis" so stale prefix text doesn't
        // interfere when the recogniser returns a growing partial transcript.
        guard let range = lowered.range(of: "jarvis", options: .backwards) else { return }
