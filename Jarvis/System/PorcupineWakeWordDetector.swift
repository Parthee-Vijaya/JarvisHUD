...
/// On-device "Jarvis" wake-word detector using Picovoice Porcupine.
///
/// ## Setup (required for the detector to actually work)
/// 1. In Xcode: **File → Add Package Dependencies…**
/// 2. Paste: `https://github.com/Picovoice/porcupine`
/// 3. Choose the **Porcupine** product and add it to the Jarvis target.
...
final class PorcupineWakeWordDetector: WakeWordDetecting {
    ...
    func start(onWake: @escaping @MainActor () -> Void) throws {
        ...
        do {
            // "jarvis" is one of Porcupine's built-in free keywords — no custom .ppn needed.
            porcupine = try Porcupine(accessKey: accessKey, keyword: .jarvis)
        ...
        isActive = true
        LoggingService.shared.log("Wake word detector started (Porcupine, 'jarvis' keyword)")
        #else
    ...
    private func feedPorcupine(_ buffer: AVAudioPCMBuffer) {
        ...
        while pendingBuffer.count >= frameLength {
            ...
            do {
                ...
                if keywordIndex >= 0 {
                    LoggingService.shared.log("Wake word detected: 'jarvis'")
                    let handler = onWake
                    Task { @MainActor in
                        handler?()
                    }
